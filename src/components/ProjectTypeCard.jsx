...
export default function ProjectTypeCard({ type, lang, selected, isAdmin, overrideUrl, onAdminImageChange, editLabel, onSelect, prefillSrc, onEditClick, onImageError }) {
  ...
  return (
    <Hoverable onClick={onSelect} style={cardStyle} hoverStyle="transform:translateY(-4px);box-shadow:0 16px 32px -12px oklch(20% 0.02 50 / 0.3);" role="button" tabIndex={0} aria-pressed={selected} onKeyDown={(e) => e.key === 'Enter' && onSelect()}>
      <EditableImage slotId={'type-' + type.key} placeholder={name} prefillSrc={prefillSrc} isAdmin={isAdmin} overrideUrl={overrideUrl} onAdminImageChange={onAdminImageChange} editLabel={editLabel} stopClickPropagation onEditClick={onEditClick} onImageError={onImageError} />
